...
**Phase A — Discovery:** Verified all existing issues (#18, #20, #21, #22, #24 valid; #19 closed). Discovered 3 new issues across duplication and error handling categories: #26 (PIN modal duplication), #27 (empty catch in retry), #28 (base64 validation).

**Phase B — Implementation (#27, #28):** 
- **PR #29 — Error handling hardening:** Fixed empty catch in admin refresh retry (admin.tsx line 421) to show error message. Added try/catch around atob in rota upload (rotas.functions.ts) with descriptive error message. 2 files changed, 7 insertions. Build ✅.
- #26 filed as `needs-triage` only — refactor needs human approval.

**Status:** 2 of 3 discovered issues auto-fixed in PR #29. 1 issue (#26) awaiting human triage. Open PRs #23, #25 still awaiting human merge. No further auto-fix issues remain.
